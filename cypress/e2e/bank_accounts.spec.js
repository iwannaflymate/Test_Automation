...
describe('Verify Bank Account flow', () => {
    ...
    it('Should be possible to delete bank account', () => {
        cy.get(account_page.sidebar_selectors.bank_accounts_btn).should('be.visible').and('have.text', 'Bank Accounts').click();
        cy.url().should('include', '/bankaccounts');
        cy.get(account_page.bank_acc_selectors.bank_acc_delete_btn).should('be.visible').and('have.text', 'Delete').click();
    ...
    it('Should not be possible to create Bank Account with invalid data and show validation messages', () => {
        cy.get(account_page.sidebar_selectors.bank_accounts_btn).should('be.visible').and('have.text', 'Bank Accounts').click();
        cy.url().should('include', '/bankaccounts');
        cy.get(account_page.bank_acc_selectors.bank_acc_create_btn).should('be.visible').and('have.text', 'Create').click();
        ...
        cy.get(account_page.onboarding.bank_name_field).click().get(account_page.onboarding.bn_field_validation).should('be.visible')
         .and('have.text', 'Enter a bank name').get(account_page.onboarding.bank_name_field).type('1234')
         .get(account_page.onboarding.bn_field_validation).should('be.visible').and('have.text', 'Must contain at least 5 characters');
        cy.get(account_page.onboarding.rn_field_validation).should('be.visible').and('have.text', 'Enter a valid bank routing number')
         .get(account_page.onboarding.routing_name_field).click().type('12345678').get(account_page.onboarding.rn_field_validation)
         .should('be.visible').and('have.text', 'Must contain a valid routing number').get(account_page.onboarding.routing_name_field)
         .click().type('1234567890').get(account_page.onboarding.rn_field_validation).should('be.visible')
    ...
    it('Should be possible to create Bank Account with valid data', () => {
        ...
        cy.wait('@graphql').its('response.statusCode').should('eq', 200);
        cy.get(account_page.bank_acc_selectors.bank_acc_delete_btn).should('be.visible').and('have.text', 'Delete');
    });
});